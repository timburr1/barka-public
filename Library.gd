...
func _ready():
	Sound.get_node("LibraryMusic").play()
	transition.visible = true
	GameState.current_screen = "res://World/Library/Library.tscn"
	$YSort/Azulejo/AnimatedSprite.playing = true
...
func _process(_delta):	
	if exit.can_see_player() and !exit_flag:
		...
		if GameState.current_day == 9 and !GameState.end_day:
			...
			GameState.previous_screen = "res://World/Library/Library.tscn"
